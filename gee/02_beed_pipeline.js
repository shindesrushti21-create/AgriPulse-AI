...
// District: Beed(Bid) - FULL RANGE 2019-2026
// ============================================


var districts = ee.FeatureCollection('FAO/GAUL/2015/level2');

// TEMPORARY CHECK - list all district names in Maharashtra
var allDistricts = ee.FeatureCollection('FAO/GAUL/2015/level2')
  .filter(ee.Filter.eq('ADM1_NAME', 'Maharashtra'));
print('All Maharashtra district names:', allDistricts.aggregate_array('ADM2_NAME'));

var studyArea = districts
  .filter(ee.Filter.eq('ADM1_NAME', 'Maharashtra'))
  .filter(ee.Filter.eq('ADM2_NAME', 'Bid'));

Map.centerObject(studyArea, 8);
Map.addLayer(studyArea, {color: 'red'}, 'Bid District Boundary');
...
Export.table.toDrive({
  ...
  description: 'beed_monthly_features_2019_2026',
  fileFormat: 'CSV'
});
